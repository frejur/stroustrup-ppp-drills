...
void test_op_arithm_add(Monetary_math_session& s,
...
void test_op_arithm_add(Monetary_math_session& s, long a, long b, long expected)
{
	Money m_a{ s.new_money(a) };
	Money m_b{ s.new_money(b) };
	Money m_expected{ s.new_money(expected) };
	test_op_arithm_add(s, m_a.amount(), m_a.currency().id,
	                   m_b.amount(), m_b.currency().id,
	                   m_expected.amount(), m_expected.currency());
}

void test_op_arithm_add(Monetary_math_session& s, Money m_a, Money m_b,
                        Money m_expected)
{
	test_op_arithm_add(s, m_a.amount(), m_a.currency().id,
	                   m_b.amount(), m_b.currency().id,
