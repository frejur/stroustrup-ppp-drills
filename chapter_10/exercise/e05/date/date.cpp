...
int ch9d::days_in_month(const int y, const Month m)
{
...
const ch9d::Day_of_the_week& min_dotw()
{
	static ch9d::Day_of_the_week dotw{
	    ch9d::Day_of_the_week::mon}; // NOTE: 1st Jan 1601
	return dotw;
}
...
bool ch9d::is_before_date(int y1, Month m1, int d1, int y2, Month m2, int d2)
{
	return (
		   (y1 < y2)
...
bool ch9d::is_before_min_date(int y, Month m, int d)
{
...
int ch9d::days_from_sofm_until_sofm(int y, Month m1, Month m2)
{
	if (m2 < m1) {
...
int ch9d::days_from_sofy_until_sofm(int y, Month m)
{
...
int ch9d::days_from_sofm_until_eofy(int y, Month m)
{
	int d{ 0 };
...
int ch9d::leap_years_from_year_span(int start_y, int end_y)
{
...
int ch9d::duration_in_days(const Date& start, const Date& end)
{
...
// -----------------------------------------------------------------------------
bool ch9d::is_date(const int y, const Month m, const int d)
{
...
bool ch9d::is_leap_year(const int y)
{
...
ch9d::Date::Date(int yy, Month mm, int dd)
    : y{yy}
    , m{mm}
    , d{dd}
{
...
std::ostream& ch9d::operator<<(std::ostream& os, const Month month)
{
	os << static_cast<int>(month);
	return os;
}

const ch9d::Date& default_date()
{
	static ch9d::Date dd{2001, ch9d::Month::jan, 1};
	return dd;
}
...
void ch9d::Date::add_days(const int n)
{
	int days { n };
	int dd{ d };
	Month mm{ m };
...
void ch9d::Date::add_months(const int n)
{
	int months{ n };
...
void ch9d::Date::add_years(const int n)
{
...
ch9d::Day_of_the_week ch9d::Date::day_of_the_week() const
{
...
std::string ch9d::dotw_to_str(const Day_of_the_week dotw)
{
...
ch9d::Date ch9d::next_workday(const Date& date)
{
...
int ch9d::weeks_per_year(const int year)
{
...
int ch9d::week_of_year(const Date& date)
{
...
bool ch9d::operator==(const Date& a, const Date& b)
{
...
std::ostream& ch9d::operator<<(std::ostream& os, const Date& date)
{
	return os << '(' << date.year()
...
std::istream& ch9d::operator>>(std::istream& is, Date& date)
{
	int y, m, d;
