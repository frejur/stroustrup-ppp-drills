...
void fct::Fct::hide()
{
	Graph_lib::Color new_col = color();
	new_col.set_visibility(Graph_lib::Color::invisible);
	set_color(new_col);
	is_hidden = true;
}
...
{
	Graph_lib::Color new_col = color();
	new_col.set_visibility(Graph_lib::Color::visible);
	set_color(new_col);
	is_hidden = false;

	if (needs_upd) {
		// Update if any changes have been made while hidden
		upd();
	}
}
...
{
	if (is_hidden) {
		// Save update for later when visible
		needs_upd = true;
		return;
	}
	double dist = (r_.r2 - r_.r1) / (count_ - 1);
	double r = r_.r1;
