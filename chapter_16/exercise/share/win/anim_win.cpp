...
constexpr int max_refresh_rate_ms{10'000};
...
void anm_win::Animated_window::start_polling(int duration_sec)
{
	...
	const double rs = rate_ms / 1'000.0;
	const double wait_sec = rs * 0.1;
	...
	while (shown() && act_count < num_act) {
		...
		passed_ms = (std::min)(dur_ms,
		                       passed_ms + static_cast<int>(t_diff * 1'000));
...
{
	const int min_d_sec{static_cast<int>((r_ms / 1'000.0) + 0.5)};
