...
Exchange_rate_lib::Exchange_rate::Exchange_rate(const Exchange_rate xr_base,
                                                const Exchange_rate xr_counter)
{
	if (xr_base.as_integer() == 0 || xr_counter.as_integer() == 0) {
		update(0);
		return;
	}

	// Determine sign
	int sign_base = (xr_base.whole < 0) ? -1 : 1;
	int sign_counter = (xr_counter.whole < 0) ? -1 : 1;
	int sign = sign_base * sign_counter;

	// Absolute values, apply sign at the end
	long long numerator = xr_base.as_integer() * sign_base;
	long long denominator = xr_counter.as_integer() * sign_counter;

	// Adjust for internal scaling
	numerator *= xr_counter.scale_factor();
	denominator *= xr_base.scale_factor();

	// Increase size of numerator to maximize precision of end result
	long long precision_scale_f{MLH::factor_until_overflow(numerator)};
	numerator *= precision_scale_f;

	// Preliminary cross rate
	long long cross{numerator / denominator};

	// Calculate increase/reduction of final result due to internal scale factors
	int whole_temp{static_cast<int>(cross / precision_scale_f)};

	long long fract_temp{
	    (whole_temp == 0) ? cross : cross - whole_temp * precision_scale_f};

	int count_d_cross{static_cast<int>(MLH::get_exp(precision_scale_f))};
	int count_d_fract{MLH::count_d(fract_temp)};
	int exp_temp{count_d_fract - count_d_cross};

	long long fract_temp_round{fract_temp};
	if (count_d_fract < MLH::XRATE_SIGNIFICANT_DIGITS()) {
		fract_temp *= MLH::exp_f(MLH::XRATE_SIGNIFICANT_DIGITS()
		                         - count_d_fract);
		fract_temp_round = fract_temp;
	} else if (count_d_fract > MLH::XRATE_SIGNIFICANT_DIGITS()) {
		fract_temp /= MLH::exp_f(count_d_fract - MLH::XRATE_SIGNIFICANT_DIGITS()
		                         - 1);
...
void XRT::Exchange_rate::update(long double xrate)
{
	...
	long double fract_as_double{(whole_temp == 0) ? xrate * sign
	                                              : xrate * sign - whole_temp};
	...
	fract_temp_round = ((fract_as_double * MLH::XRATE_FACTOR() * exp_f * 10
	                     + (5 * sign))
	                    / 10);
	if (fract_temp != fract_temp_round) {
		if (MLH::count_d(fract_temp) < MLH::count_d(fract_temp_round)) {
...
long double XRT::Exchange_rate::value() const
{
	return (whole
	        + fract
	              / static_cast<long double>(MLH::XRATE_FACTOR()
	                                         * MLH::exp_f(-exp)));
}

long long XRT::Exchange_rate::as_integer() const
{
	return (whole * MLH::XRATE_FACTOR() * MLH::exp_f(-exp) + fract);
}
...
int XRT::Exchange_rate::scale_factor() const
{
	...
	return (scale == 0) ? 1 : static_cast<int>(scale);
}
...
void XRT::Exchange_rate::flip_currencies()
{
	...
	long long internal_scale_f{scale_factor()};
	long long unit_of_one{
	    static_cast<long long>(MLH::XRATE_FACTOR() * internal_scale_f)};
	...
	int whole_temp{static_cast<int>(xrate / precision_scale_f)};
	long long fract_temp{
	    (whole_temp == 0) ? xrate : xrate - whole_temp * precision_scale_f};

	// int count_d_unit{static_cast<int>(MLH::get_exp(combined_scale_f))};
	int count_d_unit{static_cast<int>(MLH::get_exp(precision_scale_f))};
	int count_d_fract{MLH::count_d(fract_temp)};
