...
ML::Monetary_math::Monetary_math(std::istream& istr)
    : is_active{true}
    , vehicle(Currency_ID::Not_a_currency)
    , is{istr}
{
	...
	const
{
	...
	if (p.base == Currency_ID::Not_a_currency) {
		// No matching exchange rate found
		if (id_a == vehicle || id_b == vehicle
		    || vehicle == Currency_ID::Not_a_currency) {
			throw std::runtime_error("Could not fetch exchange rate");
		}
		// Attempts to create one using vehicle
		ML::Currency_pair p_a{tbl.get(vehicle, id_a)};
		if (p_a.base == Currency_ID::Not_a_currency) {
			throw std::runtime_error("Could not fetch vehicle echange rate for "
			                         "cross rate calculation");
		}
		if (p_a.base != id_a) {
			p_a.xrate.flip_currencies();
		}
		ML::Currency_pair p_b{tbl.get(vehicle, id_b)};
		if (p_b.base == Currency_ID::Not_a_currency) {
			throw std::runtime_error("Could not fetch vehicle echange rate for "
			                         "cross rate calculation");
		}
		if (p_b.base != id_b) {
			p_b.xrate.flip_currencies();
		}
		Exchange_rate_lib::Exchange_rate cross{p_a.xrate, p_b.xrate};
		return cross;
	}
