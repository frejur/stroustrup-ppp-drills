...
enum class Currency_ID {
	Not_a_currency = -1,
	USD, EUR, JPY, GBP, CAD, AUD, CHF, CNY, INR, BRL, ZAR, KRW
}; // clang-format on
...
public:
	...
	std::vector<Currency_ID> filter_currencies_convertible_with_id(
...
public:
	...
	void set_vehicle_currency(Currency_ID id) { vehicle = id; };
	void add_exchange_rate(Currency_ID id_a, Currency_ID id_b, double xrate);

	std::vector<Currency> get_currencies_convertible_with_id(Currency_ID id)
	{
		std::vector<Currency_ID> id_v
		    = tbl.filter_currencies_convertible_with_id(id);
		std::vector<Currency> cur_v;
		for (Currency_ID id : id_v) {
			cur_v.push_back(get_currency(id));
		}
		return cur_v;
	};
...
struct Monetary_math_session
{
	...
	Currency get_currency(Currency_ID id) const { return mm.get_currency(id); };
	...
	void set_vehicle_currency(Currency_ID id) { mm.set_vehicle_currency(id); };
	void add_exchange_rate(Currency_ID id_a, Currency_ID id_b, double xrate) {
	...
	std::vector<Currency> get_currencies_convertible_with_id(Currency_ID id)
	{
		return mm.get_currencies_convertible_with_id(id);
	};
	XRT::Exchange_rate get_exchange_rate(Currency_ID id_a,
	                                     Currency_ID id_b) const
	{
		return mm.get_exchange_rate(id_a, id_b);
	};
