...
// Chapter 7, Exercise 10. Modify the calculator to only operate on <int>'s

// Throw errors for over/underflow using narrow_cast.

// There are a lot of changes to the code, too many to mark in any sensible way
...
public:
	...
	int declare(string name, int val, bool is_const);
	bool is_declared(string name);
private:
	vector<Variable> var_table;
};

int Symbol_table::get(string name)
{
...
void Symbol_table::set(string name, int val)
{
...
int Symbol_table::declare(string name, int val, bool is_const)
{
...
int statement();						// read and evaluate a Statement
int declaration();					// read and evaluate a Declaration
int redefinition();					// CH07E02 redefine a variable
int expression();					// read and evaluate an Expression
...
int primary() {
	...
	case pow_token: // power of
	{
		...
		int expo = narrow_cast<int>(expression());
		t = ts.get();
		if (t.kind != ')') error("expected ')' after exponent");
		return narrow_cast<int>(pow(expr, expo));
	}
	case number:   // number
